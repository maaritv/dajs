...
const volcanosAndEruptions = outerJoinVolcanosAndEruptions(volcanos, transformedEruptions).filter(volcano => volcano.volcano_name)
...
function  outerJoinVolcanosAndEruptions(volcanos, eruptions){
    ...
    //jos tulivuori ei ole purkautunut, se lisätään tulostaulukkoon purkauksen kentät asetettuna null-arvoon.
    //silloin hasNeverErupted palauttaa true
    const volcanosWithEmptyEruptions = volcanosWithNoEruptions(volcanos, eruptions)
    //Liitetään rivit sisäliitoksesta ja ulkoliitoksesta samaan tulostaulukkoon
    const volcanosWithAndWithoutEruptions = [...volcanosAndEruptions, ...volcanosWithEmptyEruptions]
    return volcanosWithAndWithoutEruptions
}
...
function volcanosWithNoEruptions(volcanos, eruptions){
    const listOfEruptedVolcanoNames = [...new Set(eruptions.map(eruption => eruption.volcano_name))];
    
    //Sisäfunktio palauttaa true, jos tulivuoren nimi EI löydy listasta purkautuneita tulivuoria
    function hasNeverErupted(volcano){
        const hasErupted = listOfEruptedVolcanoNames.find(name=> name===volcano.volcano_name)
        return !hasErupted
    } 
    
    const listOfVolcanosNotIncludedToEruptedVolcanos = volcanos.filter(volcano=> hasNeverErupted(volcano))
    //Tyhjä purkausobjekti, joka liitetään niihin tulivuoriin, jota eivät ole koskaan purkautuneet.
    const emptyEruptionObject = {
        eruption_number: null,
        eruption_category: null,
        area_of_activity: null,
        vei: null, // Volcanic Explosivity Index (olettamuksena numero)
        start_year: null,
        start_month: null,
        start_day: null,
    ...
    const volcanosWithEmptyEruptions = listOfVolcanosNotIncludedToEruptedVolcanos.map(volcano => {
        return {...volcano, ...emptyEruptionObject}
    })

    return volcanosWithEmptyEruptions
}
